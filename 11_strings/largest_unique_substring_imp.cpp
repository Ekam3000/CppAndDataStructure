...
int longest_unique_substring(char *input)
{
    ...
    int lastindex[256]; // lastindex[256] representing when we have seen a particular character last time.
    for (int i = 0; i < 256; i++)
    {
        lastindex[i] = -1;
    }
    ...
    // for comparing last window of longest substring with previous window of longest substring and hence printing longest substring accordingly
    int currentOptionlength = next - start;
    if (currentOptionlength > longestSoFar)
    {
        longestSoFar = currentOptionlength;
